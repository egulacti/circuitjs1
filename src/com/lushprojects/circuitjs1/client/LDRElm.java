...
class LDRElm extends CircuitElm implements Command, MouseWheelHandler {
   ...
   // 	slider.addAdjustmentListener(this);
    ...
    Point ps3, ps4;
    int bodyLen, xc, yc, circleSize=20;
...
//	}
	...
	if ((flags & FLAG_IS_LDR) !=0) {
	    arrowPoint = interpPoint(point1, point2, soff / dn + .5, 16 * sign(offset));
	} else
	{
	    arrowPoint = interpPoint(point1, point2, soff / dn + .5, -36 * sign(offset));
	}
	...
	if ((flags & FLAG_IS_LDR) !=0) {
	    Point circleCenter=interpPoint(point1,point2, 0.5f);
	    xc=circleCenter.x;
	    yc=circleCenter.y;
	...
	if (sim.showValuesCheckItem.getState() && resistance1 > 0 ) {
	...
	if (n == 2) {
            ...
            ei.checkbox = new Checkbox("Resistor type is LDR", (flags & FLAG_IS_LDR) != 0);
            return ei;
	}
	return null;
    }
	...
	if (n == 2)
	    flags = ei.changeFlag(flags, FLAG_IS_LDR);
    }
